...
(function () {
  ...
  panel.innerHTML =
    ...
    `<a class="mobile-link" href="tienda.html">Tienda</a>` +
    `<a class="mobile-link" href="blog.html">Blog</a>` +
    ((SITE.redes && SITE.redes.length)
      ? `<div class="mobile-group"><span>Redes</span><div class="yrs">${
          SITE.redes.map(r => `<a href="${r.url}" target="_blank" rel="noopener">${r.nombre}</a>`).join('')
        }</div></div>`
      : '');
  ...
  /* --- pie de página con redes (en todas las páginas) --- */
  const redes = (SITE.redes || []).map(r =>
    `<a href="${r.url}" target="_blank" rel="noopener">${r.nombre}</a>`
  );
  if (SITE.email) {
    redes.push(`<a href="mailto:${SITE.email}">Correo</a>`);
  }
  const footer = document.createElement('footer');
  footer.className = 'site-footer';
  footer.innerHTML =
    `<span class="foot-copy">© ${new Date().getFullYear()} Luna Go</span>` +
    `<nav class="foot-social" aria-label="Redes sociales">${redes.join('')}</nav>`;
